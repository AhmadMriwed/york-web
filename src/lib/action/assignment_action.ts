...
export const updateExam = async (
  ...
  try {
    ...
    if (values.exam_config) {
      formData.append('exam_config[count_questions_page]', values.exam_config.count_questions_page?.toString() || "");
      formData.append('exam_config[count_return_exam]', values.exam_config.count_return_exam?.toString() || "");
      formData.append('exam_config[view_answer]', values.exam_config.view_answer?.toString() || "");
      formData.append('exam_config[view_results]', values.exam_config.view_results || "");
      formData.append('exam_config[language]', values.exam_config.language || "");
      if (values.exam_config.start_date) formData.append('exam_config[start_date]', values.exam_config.start_date.toString());
      if (values.exam_config.end_date) formData.append('exam_config[end_date]', values.exam_config.end_date.toString());
    }
    ...
    formData.append('start_form[sub_title]', values.start_form?.sub_title || "");
    formData.append('start_form[description]', values.start_form?.description || "");
    formData.append('start_form[show_condition]', values.start_form?.show_condition ? '1' : '0');
    formData.append('start_form[show_configration]', values.start_form?.show_configration ? '1' : '0');

    if (values.end_form) {
      formData.append('end_form[title]', values.end_form.title || "");
      formData.append('end_form[sub_title]', values.end_form.sub_title || "");
      formData.append('end_form[description]', values.end_form.description || "");
    ...
    if (values.exam_condition?.condition_exams_id?.length) {
      formData.append('exam_condition[condition_exams_id]', JSON.stringify(values.exam_condition.condition_exams_id));
    }

    // Send the form data to the server
    const response = await axios.post(`/assignment/exams/update-all`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
...
export const changeExamStatus = async (id: number): Promise<void> => {
  try {
     const res = await axios.patch(`/assignment/exams/${id}/toggle-status`, getAuthHeaders()); 
...
// start form : 
export const fetchStartFormById = async (id:number): Promise<StartFormType> => {
  try {
...
export const updateEvaluation = async (
  ...
  try {
    ...
    // Send the form data to the server
    const response = await axios.post(`/evaluation/evaluations/update-all`, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
...
export const changeEvaluationStatus = async (id: number): Promise<void> => {
  try {
     const res = await axios.patch(`/evaluation/evaluations/${id}/toggle-status`, getAuthHeaders()); 
...
export const deleteEvaluation = async (id: number): Promise<void> => {
  try {
     const res = await axios.delete(`/evaluation/evaluations/${id}`, getAuthHeaders()); 
