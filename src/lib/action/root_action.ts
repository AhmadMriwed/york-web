...
export const getVenuesById = async (id: number): Promise<Venue> => {
  try {
...
export const fetchUpcomingCourses = async (): Promise<Upcoming_Course[]> => {
  try {
...
export const getCoursesByVenueId = async (
  venueId: number
): Promise<Course[]> => {
  try {
    const response = await axios.get(
...
export const registration = async (data: RegistrationData): Promise<void> => {
  try {
    ...
    if (
      data.selection_training &&
      typeof data.selection_training === "object"
    ) {
      formData.append(
        "selection_training",
        JSON.stringify(data.selection_training)
      );
    }
...
export const getLatestTrainingPlan = async (): Promise<Training_Plan> => {
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to fetch training plan"
    );
  }
};
...
export const storePlanRegister = async (
  data: PlanRegisterData
): Promise<void> => {
  try {
...
export const SearchCourse = async (
  ...
  try {
    ...
    const response = await axios.get(
      ...
      }/course_ads/search?${query.toString()}`,
      {
        headers: {
...
export const fetchContactUsDataWithId = async (
  id: number
): Promise<ContactUs> => {
  try {
...
export const CertificateReview = async (values: {
  ...
  try {
    ...
    const errorMessage =
      ...
      "Failed to submit certificate review. Please try again.";
...
export const fetchFooterIcons = async (
  language: string
): Promise<FooterIcons[]> => {
  try {
    const response = await axios.get(
...
export const downloadFile = async (filePath: string) => {
  try {
    ...
    const response = await fetch(
      `/api/proxy?path=${encodeURIComponent(filePath)}`
    );
...
export const readFile = async (filePath: string): Promise<string | null> => {
  try {
    ...
    const response = await fetch(
      `/api/proxy/read?path=${encodeURIComponent(filePath)}`
    );
