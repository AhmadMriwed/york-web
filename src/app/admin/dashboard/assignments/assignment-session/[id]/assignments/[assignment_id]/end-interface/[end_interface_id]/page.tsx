...
const EndInterface = () => {
  const { end_interface_id, assignment_id, id } = useParams();
 ...
 useEffect(() => {
  ...
  return (
    <InterfaceModal interfaceData={EndInterfaceData} header={"End Interface"} interface_id={Number(end_interface_id)}  id={Number(id)} assignment_id={Number(assignment_id)} loader={loading} interf={'end'}/>
  )
}
