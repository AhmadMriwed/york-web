...
const ReactQuill = dynamic(() => import('react-quill'), { ssr: false });
import 'react-quill/dist/quill.snow.css';
import { deletedQuestionOptions, getQuestionById, updateQuestion } from '@/lib/action/exam_action';
import { toast } from 'sonner';
import { ThemeContext } from '@/components/Pars/ThemeContext';
...
const EditQuestionPage = () => {
  ...
  const [datadata, setDataData] = useState<Data>();
  ...
  useEffect(() => {
    const fetchQuestion = async () => {
      try {
        ...
        const questionType =
          apiData.question_type_id === 1 ? 'single' :
            apiData.question_type_id === 2 ? 'multi' :
              apiData.question_type_id === 3 ? 'true_false' :
                'short';

        const options = apiData.fields.map((field: any, index: number) => ({
          id: field.id,
          text: field.field,
          // isCorrect: apiData.correct_answers.some((ca: any) => ca.id === field.id)
          isCorrect: apiData.correct_answers.some((ca: any) =>
...
try {
  setQuestionData({
    ...
    type: questionType,
    required: true,
    show_grade: Boolean(apiData.show_grade),
    hint: apiData.hint,
    correct_answer_grade: apiData.correct_answer_grade,
    wrong_answer_grade: apiData.wrong_answer_grade || 0,
  });
} catch (error) {
  console.log(error);
}
  ...
  const validateForm = () => {
    ...
    newErrors.question = !questionData.question.trim() ||
  ...
  const handleCorrectAnswerChange = (index: number, id: number) => {
    const newOptions = [...questionData.options];
    ...
    // if (datadata?.fields) {
    //   const idsToDelete = datadata.fields
    //     .filter(field => field.id !== id)
    //     .map(field => field.id);
      
    //   setDeletedIds(prev => {
    //     const combined = [...prev, ...idsToDelete];
  ...
  const deleteOption = (id: number, index: any) => {
    // const deletedOption = questionData.options[id];
    // if (deletedOption.id) {
    // }
    const newOptions = questionData.options.filter((_, i) => i !== index);
    setQuestionData({ ...questionData, options: newOptions });

    if (datadata?.fields?.some(field => field.id === id)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const formData = {
      ...
      field_types: questionData.options.map(opt => 'text'),
      correct_value: questionData.options.filter(opt => opt.isCorrect).map(opt => opt.text),
      correct_answer_id: questionData.options.filter(opt => opt.isCorrect).map(opt => opt.id),
      required: questionData.required ? 1 : 0
    };
    ...
    try {
      ...
      if (deletedIds.length > 0) {
        await deletedQuestionOptions(deletedIds);
      }
                  ...
                  <div key={index} className="flex items-center gap-3">
                    {questionData.type !== 'true_false' && (
                      <input
                        type={questionData.type === 'single' ? 'radio' : 'checkbox'}
                        checked={option.isCorrect}
                        onChange={() => handleCorrectAnswerChange(index, Number(option.id))}
                        className="h-4 w-4 text-primary-color1 focus:ring-primary-color1"
                      />
                    ...
                    {questionData.type !== 'true_false' && (
                      <button
                        type="button"
                        onClick={() => deleteOption(Number(option.id), index)}
                        className="text-red-500 hover:text-red-700"
                      >
                        <Trash className="w-5 h-5" />
                      </button>
