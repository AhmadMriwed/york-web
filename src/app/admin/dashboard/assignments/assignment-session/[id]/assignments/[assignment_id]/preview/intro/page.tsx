...
const Intro = () => {
  ...
  const startExam = async () => {
    ...
    router.push(`/admin/dashboard/assignments/assignment-session/${id}/assignments/${assignment_id}/preview/questions`);
  };
            ...
            <div className="space-y-4 text-left">
              ...
              {examData?.exam_config?.condition_exams?.length! > 0 && (
                <div className="flex items-start gap-3 p-3 bg-gray-50  rounded-lg border border-gray-200 ">
                  ...
                  <span className="inline-flex items-center justify-center w-8 h-8 bg-white  border-2 border-primary-color1 rounded-full shrink-0 mt-0.5">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {examData?.exam_config?.condition_exams?.map(
                        (condition) => (
                          <span
                            key={condition.id}
                            className="px-2.5 py-1 bg-white  text-gray-700  text-sm rounded-full border border-gray-400 dark:border-gray-600 shadow-xs"
                          >
                            {condition.name}
                          </span>
                        )
