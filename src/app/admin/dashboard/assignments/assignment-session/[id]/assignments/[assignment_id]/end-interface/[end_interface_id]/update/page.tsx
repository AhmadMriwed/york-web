...
// import { EditValidationEnd } from "@/schemas/interface";
// import { z } from "zod";
...
//         resolver: zodResolver(EditValidationEnd),
//         defaultValues,
//     });
...
//                     <IoArrowBackSharp className="text-primary-color1 text-lg sm:mr-4 sm:text-xl cursor-pointer"
//                         onClick={() => router.back()}
//                     />
...
const Page = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            ...
            try {
                if (!end_interface_id) {
                    throw new Error("Missing interface ID in URL");
                }
                const data = await getEndInterface(Number(end_interface_id));
                setInterfaceData(data?.data);
            } catch (err) {
    ...
    return (
        <div className="px-1 sm:px-4 pt-3 sm:pt-5">
            <div className="flex justify-between items-start mb-5 pt-2">
                <Header className="flex justify-start items-center gap-2 max-sm:pt-1 max-sm:px-3 text-[var(--primary-color1)] hover:text-[var(--primary-color2)]">
                    <IoArrowBackSharp 
                        className="text-primary-color1 text-lg sm:text-xl cursor-pointer sm:mr-4"
                        onClick={() => router.back()}
                    />
                    <h3 className="text-[20px] sm:text-2xl font-semibold tracking-wide">Update Interface</h3>
                </Header>
