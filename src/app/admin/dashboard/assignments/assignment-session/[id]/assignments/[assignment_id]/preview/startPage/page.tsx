...
const createValidationSchema = (fieldRequirements: any) => {
  ...
  fieldRequirements.forEach((field: any) => {
    switch (field.name) {
      case "first_name":
        schema.first_name = z.string().optional();
        break;
      case "last_name":
...
const Page = () => {
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      ...
      setIsModalOpen(false);
      router.push(
        `/admin/dashboard/assignments/assignment-session/${id}/assignments/${assignment_id}/preview/intro`
      );
    } catch (error: any) {
      // toast.dismiss(toastId);
  ...
  const handleIdVerification = () => {
    if (idNumberInput.trim()) {
      router.push(
        `/admin/dashboard/assignments/assignment-session/${id}/assignments/${assignment_id}/preview/result?id_number=${idNumberInput}`
      );
  ...
  return (
    <>
      ...
      >
        <p className="mb-4">
          Becauce you are an admin the field will be unRequired.
        </p>
        <Form {...form}>
          <form
