...
const EditQuestionPage = () => {
    ...
    const [deletedCorrectAnswerIds, setDeletedCorrectAnswerIds] = useState<number[]>([]);
    ...
    const validateForm = () => {
        const newErrors = {
            question: !questionData.question.trim() || questionData.question.trim() === '<p><br></p>',
    ...
    const handleTypeChange = (newType: 'single' | 'multi' | 'true_false' | 'short' | 'long') => {
        ...
        setQuestionData({
            ...
            type: newType,
            options: newOptions,
            correct_answer_grade: newType === 'true_false' ? questionData.correct_answer_grade : 0,
            ...
            newOptions.forEach((opt, i) => {
                if (i === index) {
                    ...
                    if (apiData?.correct_answers.some(ca =>
                        ...
                        if (correctAnswer) {
                            setDeletedCorrectAnswerIds(prev => [...prev, correctAnswer.id]);
                        }
                    }
                } else {
                    if (opt.isCorrect) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const formData = {
            ...
            field_id: questionData.options.map(opt => opt.id || null),
        ...
        console.log('Deleting options:', deletedOptionIds);
        console.log('Deleting correct answers:', deletedCorrectAnswerIds);

        const toastId = toast.loading('Updating question...');
        try {
            // Delete operations if needed
            if (deletedCorrectAnswerIds.length > 0) {
                await deletedQuestionCorrectAnswers(deletedCorrectAnswerIds);
            }
                    ...
                    <div>
                        ...
                        <div className="dark:bg-gray-700 rounded">
                            <ReactQuill
                                ...
                                className={`dark:text-white ${mode === 'dark' ? 'ql-dark' : ''}`}
                                modules={modules}
                            />
                        </div>
                    </div>
                                ...
                                {questionData.options.map((option, index) => (
                                    <div key={index} className="flex items-center gap-3">
                                        ...
                                        {questionData.type !== 'true_false' && (
                                            <button
                                                type="button"
                                                onClick={() => deleteOption(option.id, index)}
...
//     const handleTypeChange = (newType: 'single' | 'multi' | 'true_false' | 'short' | 'long') => {
//         // Only track for deletion when changing to/from true_false or between question types and answer types
//         const isChangingToTrueFalse = newType === 'true_false';
//         const isChangingFromTrueFalse = questionData.type === 'true_false';
//         const isChangingBetweenQuestionAndAnswer = 
//             (['single', 'multi'].includes(questionData.type) && ['short', 'long'].includes(newType)) ||
//             (['short', 'long'].includes(questionData.type) && ['single', 'multi'].includes(newType));

//         if (apiData && (isChangingToTrueFalse || isChangingFromTrueFalse || isChangingBetweenQuestionAndAnswer)) {
//             // Add all existing field IDs to deletedOptionIds
//             const currentFieldIds = apiData.fields.map(field => field.id);
//             setDeletedOptionIds(prev => [...prev, ...currentFieldIds]);
...
//             ];
//         } else if (['short', 'long'].includes(newType)) {
//             if (['short', 'long'].includes(questionData.type)) {
//                 // Keep existing answer if changing between short and long
//                 newOptions = questionData.options.length > 0 
//                     ? [{ ...questionData.options[0], isCorrect: true }] 
//                     : [{ text: '<p><br></p>', isCorrect: true }];
//             } else {
//                 // Initialize with empty delta structure for Quill
//                 newOptions = [{
//                     text: '<p><br></p>',
...
//                 // Keep existing options when changing between single and multi
