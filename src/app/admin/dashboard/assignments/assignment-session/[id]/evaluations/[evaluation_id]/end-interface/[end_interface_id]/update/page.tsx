...
const EditForm = ({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      setIsUploading(true);
      try {
        const response = await uploadFileToEndForm(
          Number(end_interface_id),
          file
        );
...
const Page = () => {
  ...
  const [interfaceData, setInterfaceData] = useState<EndInterfaceType | null>(
    null
  );
  ...
  const handleSave = async (formData: FormData) => {
    ...
    try {
      const response = await updateEndInterface(
        Number(end_interface_id),
        formData
      );
      console.log("API Response:", response);
      ...
      router.push(
        `/admin/dashboard/assignments/assignment-session/${id}/evaluations/${evaluation_id}/end-interface/${end_interface_id}`
      );
    } catch (error: any) {
      console.error("Submission Error:", error);
      toast.error("Oops! Something went wrong", {
    ...
    <div className=" px-1 sm:px-4 pt-3 sm:pt-5 ">
      <div className="flex justify-between items-start mb-5 pt-2">
        <Header className="flex justify-start items-center gap-2 max-sm:pt-1 max-sm:px-3 text-[var(--primary-color1)] hover:text-[var(--primary-color2)]">
          <IoArrowBackSharp
            className="text-primary-color1 text-lg sm:text-xl cursor-pointer sm:mr-3"
            onClick={() => router.back()}
          />
