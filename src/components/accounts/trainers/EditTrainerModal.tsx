...
import { createTrainer, editeTrainer } from "@/store/adminstore/slices/accounts/trainersSlice";
...
export default function EditTrainerModal({
  ...
  useEffect(() => {
    if (open && trainerId) {
      ...
      dispatch(getSingleUser(`trainerAccountRequests/${trainerId}`))
        .then((response: any) => {
          setTrainerData(response.payload);
          setLoading(false); 
        })
        .catch((error: any) => {
        ...
        setAccountStatus(trainerData?.account_status);
        setUserStatus(trainerData?.status.status);
    }
  ...
  const submithandler = async (values: any, actions: any) => {
    ...
    try {
      ...
      await dispatch(editeTrainer({trainerId,formData}));
    ...
    >
      <Formik
        initialValues={{
          ...
          image: "",
          account_status: trainerData?.account_status || "",
          status: trainerData?.status?.status || "",
          trainer_type_id: trainerData?.trainer_type_id || 1,
          birth_date: trainerData?.birth_date || "",
          gender: trainerData?.gender || "",
          phone_number: trainerData?.phone_number || "",
          categories: trainerData?.categories || [],
          digital_signature: trainerData?.digital_signature || "",
          location: {
            latitude: trainerData?.location?.latitude || position.lat,
            longitude: trainerData?.location?.longitude || position.lng,
          },
          domains: trainerData?.domains || "",
          about_me: trainerData?.about_me || "",
        }}
            ...
            <Form className="flex flex-wrap flex-col sm:flex-row gap-[15px] justify-between">
              <div className="basis-[48%]">
                ...
                {props.errors.categories && props.touched.categories && (
                  <div className="pl-[5px] ml-[10px] mb-[10px] text-red-600">
                    {String(props.errors.categories)}
                  </div>
                )}
                <label className="block pl-[5px] text-[#888] mb-1">
                  About Me
              ...
              <div className="basis-[48%]">
                ...
                {props.errors.location && props.touched.location && (
                  <div className="pl-[5px] ml-[10px] mb-[10px] text-red-600">
                    {String(props.errors.location.latitude)}
                  </div>
                )}
                <LocationModal
                ...
                {props.errors.account_status &&
                  props.touched.account_status && (
                    <div className="pl-[5px] ml-[10px] mb-[10px] text-red-600">
                      {String(props.errors.account_status)}
                    </div>
                  )}
                ...
                {props.errors.trainer_type_id &&
                  props.touched.trainer_type_id && (
                    <div className="pl-[5px] ml-[10px] mb-[10px] text-red-600">
                      {String(props.errors.trainer_type_id)}
                    </div>
                  )}
                <CustomInput
                  label="Birthdate"
                  ...
                  {props.errors.digital_signature &&
                    props.touched.digital_signature && (
                      <div className="pl-[5px] ml-[10px] mt-[10px] text-red-600">
                        {String(props.errors.digital_signature)}
                      </div>
                    )}
                  <button
                    onClick={() => {
