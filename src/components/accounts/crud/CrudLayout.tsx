...
import ExportImportContainer from "@/components/crud/ExportImportContainer";
...
export default function CrudLayout({
  ...
  const handleRowSelected = useCallback(
    ({ selectedRows }: { selectedRows: any }) => {
      // if (factor === "id") {
        setSelectedRowsIds(selectedRows.map((row: any) => row.user_id));
        setSelectedRowsUserIds(selectedRows.map((row: any) => row.id));
  ...
  const contextActions = useMemo(() => {
    const handleDelete = () => {
      if (selectedRowsUserIds.length > 0) {
        dispatch(actionForDelete({user_ids: selectedRowsUserIds}));
